    ...
    size_t const numMovedRoutes)
{
    ...
    // MODIFY: Introduce a small bias to favor routes with smaller size when choosing the movement direction
    // This tends to maintain route structure and reduce the number of re-planned clients.
    // The bias helps to keep routes shorter and potentially improves the penalised cost.
    auto computeBias = [&](int diff, size_t sizeA, size_t sizeB) {
        return diff + (static_cast<int>(sizeA) - static_cast<int>(sizeB)) / 10;
    };
    ...
    while (true)
    {
        ...
        // MODIFY: Apply small bias towards routes with smaller size to promote stability and reduce cost
        auto sizeALeft = routesA[(startA - 1 + nRoutesA) % nRoutesA].size();
        auto sizeARight = routesA[(startA + numMovedRoutes) % nRoutesA].size();
        auto sizeBLeft = routesB[(startB - 1 + numMovedRoutes) % nRoutesB].size();
        auto sizeBRight = routesB[(startB + numMovedRoutes) % nRoutesB].size();

        int biasALeft = computeBias(differenceALeft, sizeALeft, sizeARight);
        int biasARight = computeBias(differenceARight, sizeARight, sizeALeft);
        int biasBLeft = computeBias(differenceBLeft, sizeBLeft, sizeBRight);
        int biasBRight = computeBias(differenceBRight, sizeBRight, sizeBLeft);

        int const bestDifference = std::min({biasALeft,
                                             biasARight,
                                             biasBLeft,
                                             biasBRight});
        ...
        else if (bestDifference == biasARight)
        {
            for (Client c : routesA[startA])
                selectedA[c] = false;
        ...
        else if (bestDifference == biasBLeft)
        {
        ...
        else if (bestDifference == biasBRight)
        {
            for (Client c : routesB[startB])
                selectedB[c] = false;
