...
class OVRPBLEnv:
    ...
    def get_random_problems(self, batch_size, problem_size, normalized=True):
        ...
        if problem_size == 20:
            ...
            demand_scaler = 80   # 已修正为最新 NCO 基准
        elif problem_size == 400:
            demand_scaler = 100  # 遵循 5*sqrt(N) 数学规律
        elif problem_size == 500:
            demand_scaler = 100  # 强制对齐 LEHD/ReLD 公开基准
        elif problem_size == 600:
            demand_scaler = 125  # 遵循 5*sqrt(N) 数学规律
        elif problem_size == 800:
            demand_scaler = 140  # 遵循 5*sqrt(N) 数学规律
        elif problem_size == 1000:
            demand_scaler = 250  # 极限长程规划压力测试标尺
        else:
            raise NotImplementedError
