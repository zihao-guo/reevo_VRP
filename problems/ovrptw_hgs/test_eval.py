...
from problems.ovrptw_hgs.eval import REPO_ROOT, calculate_exact_ovrptw_cost
...
def test_calculate_exact_ovrptw_cost_matches_open_baseline_solution():
    instance_path = REPO_ROOT / "data" / "generated" / "OVRPTW" / "101" / "n101-0001.vrp"
    solution_path = REPO_ROOT / "data" / "opt" / "OVRPTW" / "101" / "pyvrp" / "n101-0001.sol"

    routes = load_stub_routes(solution_path)
    expected_cost = load_expected_cost(solution_path)

    assert math.isclose(
        calculate_exact_ovrptw_cost(instance_path, routes),
        expected_cost,
        rel_tol=0.0,
        abs_tol=1e-6,
    )
