...
def load_problem_cfg(root_dir: Path) -> dict:
    cfg_path = root_dir / "cfg" / "problem" / "vrpb_hgs.yaml"
...
def load_candidate_code(root_dir: Path, candidate_path: str | None) -> str:
    source_path = Path(candidate_path) if candidate_path else root_dir / "problems" / "vrpb_hgs" / "gpt.py"
...
def calculate_exact_vrpb_cost(instance_path: Path, routes) -> float:
...
def solve_instance(
    ...
    obj = calculate_exact_vrpb_cost(dataset_dir / instance_name, result.best.routes())
    ...
    # //modify Optimise against the original HGS baseline stored under data/opt/VRPB/101/pyvrp.
...
def main() -> None:
    ...
    if candidate_path is None:
        if generated_output_path is None:
            raise RuntimeError(
                "REEVO_GENERATED_OUTPUT must be set when exporting the final VRPB HGS candidate."
            )
    ...
    with tempfile.TemporaryDirectory(prefix="vrpb-hgs-pyvrp-") as sandbox_dir:
