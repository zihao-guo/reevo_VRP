...
def load_problem_cfg(root_dir: Path) -> dict:
    cfg_path = root_dir / "cfg" / "problem" / "vrpl_hgs.yaml"
...
def load_candidate_code(root_dir: Path, candidate_path: str | None) -> str:
    source_path = Path(candidate_path) if candidate_path else root_dir / "problems" / "vrpl_hgs" / "gpt.py"
...
def calculate_exact_vrpl_cost(instance_path: Path, routes) -> float:
...
def solve_instance(
    ...
    obj = calculate_exact_vrpl_cost(dataset_dir / instance_name, result.best.routes())
...
def main() -> None:
    ...
    if candidate_path is None:
        if generated_output_path is None:
            raise RuntimeError(
                "REEVO_GENERATED_OUTPUT must be set when exporting the final VRPL HGS candidate."
            )
    ...
    with tempfile.TemporaryDirectory(prefix="vrpl-hgs-pyvrp-") as sandbox_dir:
